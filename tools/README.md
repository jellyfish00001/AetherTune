...
- `seed-vc-setup.ps1`：先唯讀檢查 Python 3.10/Tcl-Tk、固定 repo revision、非空 source/config 檔與 offline/realtime checkpoint、HF cache 必要 snapshots 與 ModelScope VAD，再建立獨立 environment；缺項先完整列出，不 clone source 或下載 weights。`-PreflightOnly` 只讀檢查。
- `seed-vc-setup-preflight-regression.ps1`：用 `$env:TEMP` 合成 fixture；第一個一般 setup invocation 以本機 Python/Tcl/Tk shim 通過 runtime probe，再確認空 source/config 與無效 checkpoint 在 pip／venv mutation 前 `BLOCKED`；第二個 invocation 單獨檢查缺少 Python。沒有呼叫真實 Python、讀取權重、啟動裝置或安裝套件。
