...
- `vcclient-rvc-probe.ps1`：對已啟動的 VCClient REST endpoint 做短 WAV → RVC chunk probe，保存 input/output（若有）與 JSON 證據；HTTP 500、CUDA exception 或沒有輸出時回傳非零並保留 `BLOCKED`，不會把 candidate 模型升為 ready。
