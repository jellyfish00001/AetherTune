...
- `audio_runner_failure.py`：不依賴模型／音訊套件的 runner 啟動保護，負責解析 `--output`、清除 stale 輸出與寫入失敗 manifest。
...
- `audio-runner-entry-regression.py`：TTS runner 的 dependency-safe import、`--output`／`--output=`、parse failure 與 stale manifest regression。
