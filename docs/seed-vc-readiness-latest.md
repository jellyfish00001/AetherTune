...
Seed-VC setup、一般使用者 GUI launcher 與 evidence validator 已有可重現操作路徑。Python 3.10.11 的 Tk 問題根因是 `tcl\tk8.6\pkgIndex.tcl` 以相對路徑載入 `..\..\bin\tk86t.dll`，但官方 Python 安裝中的 DLL 位於 `DLLs\tk86t.dll`；Tcl/Tk script (`init.tcl`、`tk.tcl`) 本身存在。官方 Tcl/Tk MSI repair exit `0`（repair log SHA-256 `364414613B6E315AFEF0349FC3E1A56B6C421F6CD31B74EE58E53F50D6F2A3BE`）後，主控依既有安裝授權建立 per-user `Python310\bin` junction 指向 `DLLs`，未覆寫既有路徑或修改 DLL。之後一般 Windows user session 的 Tcl 8.6.12、Tk package 8.6.12、setup preflight 與 GUI preflight 均 `PASS`；GUI 視窗與音訊 stream 沒有啟動。這是目前主機的修復後驗證，不證明新機／新使用者可直接 bootstrap。受限 sandbox 的 Tcl probe 仍是獨立環境結果，不得拿來推翻 host-session PASS，也不得解讀成主機 runtime 失敗。
...
| `PASS` | GUI 啟動架構 | launcher 將官方 GUI 放在 `artifacts/seed-vc/gui-session/` runtime overlay，配置其所需 `configs/hifigan.yml` 與 `configs/inuse/config.json`；model cache junction 指向已檢查的本機 HF snapshots；VAD 由 bootstrap 在程序內映射至完整 ModelScope cache，並停用隱式更新。不寫第三方 source、原 cache 或 user profile。 |
| `PASS` | 目前主機 setup／GUI preflight | 官方 Tcl/Tk MSI repair exit `0`；核對 Python 3.10.11 安裝下 `tk86t.dll` 實在 `DLLs`，而 `pkgIndex.tcl` 預期 `bin`；在未覆寫既有目標時建立 per-user `Python310\bin` junction → `DLLs`。其後 Tcl/Tk 8.6.12、setup `-PreflightOnly` exit `0` 與 GUI `-PreflightOnly` exit `0` 通過，CUDA、MME input/output inventory、checkpoint/VAD hashes 已核對。GUI 與 stream 未開啟；此結果不代表另一台主機或新使用者 bootstrap 通過。 |
...
| 命令 | 本輪結果 | 分類意義 |
|---|---|---|
| `& .\.venv\Scripts\python.exe .\tools\live-gate-regression.py` | exit `0`; regression `PASS` | outer-only timing/continuity/human identity/hash/ratings、metrics source type mismatch、缺 rating 的 PASS review 與錯誤人評 WAV hash=`BLOCKED`；outer-only synthetic source 與完全缺 review record=`WAITING`；明確 review status WAITING=`LIVE_CANDIDATE`；missing/stale/audio hash/model/route/hardware/zero=`BLOCKED`；首包 >5s=`OFFLINE`。 |
| `& .\.venv\Scripts\python.exe .\tools\audio-rack-evidence-regression.py` | exit `0`; regression `PASS` | paired fixture 僅能 rack `PASS`；完整 timing/continuity/paired run/output hash/delta mismatch=`BLOCKED`；unpaired=`WAITING`；missing/hash/route identity/silence=`BLOCKED`；>5s=`OFFLINE`；獨立合成 fixture 交給 LIVE gate 仍 `WAITING`。 |
...
| Python AST + 2 JSON schema parse | exit `0`; `PASS` | 對 live/rack validators、regressions、GUI bootstrap 做 Python AST parse；兩個 JSON schema 可解析；未替代 schema semantic validator。 |
| PowerShell 7 AST parser：GUI launcher/helper/overlay regression、offline runner/preflight regression | exit `0`; 五檔 `PARSE_PASS` | 語法檢查，不是 GUI/device/runtime E2E。setup script 未於本輪變更。 |
...
| `& .\tools\seed-vc-setup.ps1 -PreflightOnly`（一般 Windows user session） | exit `0`; `PASS` | 修復後 Tcl 8.6.12 與 Tk package 8.6.12 可載入；setup 預檢通過。 |
| `pwsh -NoProfile -File .\tools\seed-vc-gui-run.ps1 -PreflightOnly`（一般 Windows user session） | exit `0`; `PASS` | CUDA/RTX 5060 Ti、MME mic/output inventory、realtime checkpoint 與 ModelScope VAD hashes 均核對；沒有啟動 GUI 視窗或 audio stream。 |
