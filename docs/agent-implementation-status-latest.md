...
- Seed-VC 官方 GUI callback user-flow：受控非沙箱環境實測 `PASS`；使用 MME `麥克風 (HyperX QuadCast S)` → CUDA Seed-VC → `CABLE Input (VB-Audio Virtual C)`，四個 reference 都產生 finite／非零 WAV 且與 deterministic male input 不同；同步 WASAPI `CABLE Output` capture 也四案 `PASS`，證明 backend output 穿過 VB-CABLE。artifact／metrics 見 `artifacts/seed-vc/gui-userflow/phase-20260922-cable-loopback/gui-userflow-report.json`；這不是人工聽測、Light Host full-chain 或完整 LIVE evidence。
