...
| 3 | Seed-VC realtime tiny、長音檔 | 60 秒 headless `PASS`；官方 GUI callback user-flow `PASS`；完整 live gate `WAITING` | 離線男女互轉、60 秒長檔、200 block headless GPU stream 可用；官方 GUI 已以 CUDA + MME duplex stream 產生四組 reference 的 finite／非零輸出，且都與輸入不同 | 尚未完成真實聽感、virtual route loopback、10 分鐘以上 realtime 與完整 mic → backend → rack → route latency；GUI wrapper 的首次 warm-up 約 1.37 秒推論、另保留 15 秒首 case grace，不可直接當 LIVE latency | [`seed-vc-verification-latest.md`](seed-vc-verification-latest.md)、[`seed-vc-gui-userflow-test.py`](../tools/seed-vc-gui-userflow-test.py) |
...
| Common `audio-rack/` | `PLANNED`／`WAITING` | 建立 presets、plugin-profiles、routing、benchmarks 契約；保留現有 Light Host／Graillon／VB-CABLE／Voicemeeter | 實體 bypass/full-chain、plugin Δ latency、loopback 尚未完成 |
...
- Seed-VC GUI：`--preflight` 在非沙箱環境已 `PASS`，可重跑官方 GUI callback user-flow；下一步是把 VB-CABLE output 接入 audio-rack／virtual route，保存 loopback 與 timing，再做 10 分鐘穩定性與人工聽測。不得以目前 callback user-flow `PASS` 取代完整 LIVE evidence。
