...
| 3 | Seed-VC realtime tiny、長音檔 | headless `PASS`；裝置 E2E `WAITING` | 離線男女互轉、60 秒長檔、headless GPU block 可用 | 尚未以官方 GUI、PortAudio 麥克風和實際輸出裝置完成長時間 realtime | [`seed-vc-verification-latest.md`](seed-vc-verification-latest.md) |
...
| 6 | Breeze `sox`、`flash-attn`、`fast-all` | `fast-all PASS`；SoX／flash-attn `WAITING` | Breeze eager 與 `-FastAll` 都能產生 24 kHz WAV | WSL 使用者沒有非互動 sudo，SoX 未安裝；`flash-attn==2.8.3` 尚未成功 import；人工聽測未完成 | [`breeze-tts2-verification-latest.md`](breeze-tts2-verification-latest.md) |
