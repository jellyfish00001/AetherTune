...
- 最新綜合唯讀檢查：`docs/wiring-verification-latest.md`，目前 `PASS=17 / WAITING=4 / BLOCKED=1`；VB-CABLE 與 Voicemeeter B1 synthetic route 已 PASS，WAITING 包含 VCClient embedded CUDA、Light Host Graillon chain、完整 UI health 與 ONNX CUDA provider；唯一 BLOCKED 是四組角色模型 metadata／ready gate 未完成。
