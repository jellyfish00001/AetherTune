...
`offline-v1` 已在本機 RTX 5060 Ti 上完成男聲／女聲雙向實際推論，並完成 60 秒長音檔測試；`realtime-tiny` 已完成 3 block smoke 與 200 block／60 秒連續 headless GPU streaming。官方 GUI callback user-flow 也已在受控非沙箱環境實測通過四組 reference，但這些仍不等同 virtual route loopback、人工聽測或完整 `LIVE <= 5s`。
...
| virtual route loopback／完整 mic E2E | WAITING | callback 輸入是 deterministic WAV，尚未保存 VB-CABLE／Voicemeeter loopback、first-packet timing、dropout／underrun 與人工聽測 | `docs/live-gate.md` 契約；尚無完整 evidence |
...
- 官方 GUI／PortAudio callback user-flow 已通過，但 deterministic callback 不等於實體麥克風內容已經過完整輸入；仍需 loopback、實際 route、人工聽測、10 分鐘以上穩定性與 underrun/dropout 證據。
